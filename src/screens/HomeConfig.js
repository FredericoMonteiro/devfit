...
const Page = (props) => {
    ...
    const toggleWorkoutDay = (d) => {
        ...
        if(newWorkoutDays.includes(d)) {
            if(newWorkoutDays.length == 1) {
                alert('Tens de treinar pelo menos um dia.');
                return;
            }
    ...
    return (
        <Container>
            ...
            <Label>Dias em que tens treinos:</Label>
            <ListArea>
            ...
            <Label>Queres apagar tudo?</Label>
            <ResetButton title="Apagar Tudo" onPress={resetAction}/>
        </Container>
    );
}
